...
class ModelGenerator extends AbstractGenerator
{
    ...
    {
        $accestors = collect(explode(PHP_EOL . '        ', $this->getAccessors()));
        return $this->getRelationMethods()
            ->filter(fn (ReflectionMethod $method) => $this->isManyRelation($method))
            ->reject(function (ReflectionMethod $method, string $property) use ($accestors) {
                // if there is many many relation that has accessor as well, reject this from output
                return $accestors->contains(fn (string $accestor) => Str::contains($accestor, 'readonly ' . Str::snake($method->getName()) . '_count'));
            })
            ->map(function (ReflectionMethod $method) {
                return (string) new TypeScriptProperty(
